/**
 * Swagger/OpenAPI documentation for the auth routes.
 *
 * This file has no runtime effect — it only exists so swagger-jsdoc can
 * parse the `@swagger` JSDoc blocks below. See `src/config/swagger.ts` for
 * the `apis` glob that picks this file up, and `src/routes/auth-route.ts`
 * for the actual route definitions.
 */
